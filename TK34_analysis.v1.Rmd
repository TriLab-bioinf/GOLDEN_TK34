...
# Load data
```{r Loading data}
all <- read.delim2("./data/counts.txt", sep = "\t", header = TRUE, row.names = 1, comment.char = c("#") )
...
metadata<-  as.data.frame(metadata[match(colnames(all), rownames(metadata)), ])
rownames(metadata) <- colnames(all)
...
dds_res['all_data'] <- dds.all #[ , dds.all$Tissue=="all_data"]

rlog_res <- list()
rlog_res['all_data'] <- rlog(dds_res[['all_data']], blind=FALSE)
...
for (my_tissue in names(rlog_res)){
  ...
  # PC1 - PC2
  PC_coords = c(1,2)
  pcaData <- plotPCA_anycoord(object = rlog.one, intgroup=c("Genotype"), returnData=TRUE, PC_coords = PC_coords)
  percentVar <- round(100 * attr(pcaData, "percentVar"))
  y.coords = c(min(pcaData$PC1, pcaData$PC2), max(pcaData$PC1, pcaData$PC2))
  x.coords = y.coords
  
  p12 <- ggplot(data = d, aes_string(x = "PC1", y = "PC2", color = "group", label = "name")) +
    geom_point(size = 3) + 
  ...
  print(p12)
  ggsave(paste0("Plots/pca_",my_tissue,"genotype.pdf"), plot = p12)
  ...
  y.coords = c(min(pcaData$PC1, pcaData$PC2), max(pcaData$PC1, pcaData$PC2))
  x.coords = y.coords
  p1 <- ggplot(data = d, aes_string(x = "PC1", y = "PC2", color = "group", label = "name")) +
    geom_point(size = 3) + 
    xlab(paste0("PC",PC_coords[1],": ", round(percentVar[1]), "% variance")) + 
    ylab(paste0("PC",PC_coords[2],": ", round(percentVar[2]), "% variance")) + 
    coord_fixed() + 
    ...
    labs(title = my_tissue) 
  print(p1)
  ggsave(paste0("Plots/pca_",my_tissue,"genotype.pdf"), plot = p1)
  ...
  p13 <- ggplot(data = d, aes_string(x = "PC1", y = "PC2", color = "group", label = "name")) +
    geom_point(size = 3) + 
    xlab(paste0("PC",PC_coords[1],": ", round(percentVar[1]), "% variance")) + 
    ylab(paste0("PC",PC_coords[2],": ", round(percentVar[2]), "% variance")) + 
    coord_fixed() + 
    ...
    labs(title = my_tissue) 
  print(p13)
  ggsave(paste0("Plots/pca_",my_tissue,"_PC",PC_coords[1],PC_coords[2],"_genotype.pdf"), plot = p13)
...
for (my_tissue in names(dds_res)){
  ...
  res_genotype_KO_vs_WT <- lfcShrink(dds_res[[my_tissue]], coef = my_contrasts[2], type = "ashr", )
  
  summary(res_genotype_KO_vs_WT, alpha = 0.05)
  
  # Sort results by Log2FC
  res_genotype_KO_vs_WT_sorted <- sort_and_write_res_table(result_table = res_genotype_KO_vs_WT, file_name = paste0("DE_",my_tissue,"_KO_vs_WT")
                                                           ) 
...
```{r}
VennDiagram::venn.diagram(x = geneids.DE, 
  category.names = names(geneids.DE), 
...
for (i in names(DE_results)){
  ...
  ########################################
  # Fetch gene sets
  msig_h <- msigdbr(species = "Danio rerio", category = "H") %>% dplyr::select(gs_name, ensembl_gene, )
 ...
 run_overrepresentation_analysis(gene_set = msig_h, dds_res = DE_results[[i]], analysis_name = my_prefix, gs_name = "H", type = "general")
  ...
  ###########################################################################
  # Fetch gene sets
  msig_c3_gtrd <- msigdbr(species = "Danio rerio", category = "C3", subcategory = "TFT:GTRD") %>% dplyr::select(gs_name, ensembl_gene, )
    # m3.mirdb.v2022.1.Mm.entrez.gmt
  msig_c3_tft <- msigdbr(species = "Danio rerio", category = "C3", subcategory = "TFT:TFT_Legacy") %>% dplyr::select(gs_name, ensembl_gene, )
  ...
  # Run Gene Set Enrichment Analysis
  gsea_result.gtrd[i] <- run_enrichment_analysis(gene_set = msig_c3_gtrd, geneList = geneList, analysis_name = my_prefix, gs_name = "C3_TFT_GTRD", type = "general", no_plots = SKIP_GSEA_PLOTS, adj_p_cutoff = 1)
  gsea_result.tft[i] <- run_enrichment_analysis(gene_set = msig_c3_tft, geneList = geneList, analysis_name = my_prefix, gs_name = "C3_TFT_Legacy", type = "general", no_plots = SKIP_GSEA_PLOTS, adj_p_cutoff = 1)
  ...
  ##########################################################
  # Fetch gene sets
  msig_c2_reactome <- msigdbr(species = "Danio rerio", category = "C2", subcategory = "CP:REACTOME") %>% dplyr::select(gs_name, ensembl_gene, )

  # Run overrepresentation analysis
  run_overrepresentation_analysis(gene_set = msig_c2_reactome,  dds_res = DE_results[[i]], analysis_name = i, gs_name = "C2_CP_REACTOME", type = "reactome") # using type = "general to avoid error in treeplot
  ...
  # Generate barplot for paper
    ...
    ##########################################################
  ## C2 curated gene sets => CP:KEGG Canonical pathways
  ##########################################################
  # Fetch gene sets
  msig_c2_kegg <- msigdbr(species = "Danio rerio", category = "C2", subcategory = "CP:KEGG") %>% dplyr::select(gs_name, ensembl_gene, )
  
  # Run overrepresentation analysis
  run_overrepresentation_analysis(gene_set = msig_c2_kegg,  dds_res = DE_results[[i]], analysis_name = i, gs_name = "C2_CP_KEGG", type = "general") # using type = "general to avoid error in treeplot
  
  # Run Gene Set Enrichment Analysis
  gsea_result.kegg[i] <- run_enrichment_analysis(gene_set = msig_c2_kegg, geneList = geneList, analysis_name = my_prefix, gs_name = "C2_CP_KEGG", type = "general", no_plots = SKIP_GSEA_PLOTS)
  
  # Generate barplot for paper
    # PAPER_plot_gsea(gsea_result = gsea_result.kegg,
    #                 comparison_id = i,
    #                 analysis_type = paste0("C2_CP_KEGG"),
    #                 my_x_label = "KEGG Pathways",
    #                 my_path = "./PAPER")
}
```
...
```{r}
for (my_tissue in c('all_data','liver')){
